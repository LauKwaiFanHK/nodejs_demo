'use strict';
const fs = require('fs');
const zmq = require('zeromq');

// Socket to reply to client requests
const responder = zmq.socket('rep');

// Handle incoming requests
responder.on('message', data => {

    // Parse the incoming message
    const request = JSON.parse(data);
    console.log(`Received request to get: ${request.path}`);

    // Read the file and reply with content
    // reply by reciving the request, processing it and sending it back a response
    fs.readFile(request.path, (err, content) => {
        console.log('Sending response content.');
        if (err) throw err; // error occurs when no file is found
        console.log(content); // content is undefined if no file is found at the requested path
        responder.send(JSON.stringify({
            content: content.toString(), // if content is undefined, toString() method not works, this program breaks
            timestamp: Date.now(),
            pid: process.pid
        }));
    });
});

// Listen on TCP port 60401
responder.bind('tcp://127.0.0.1:60401', err => {
    console.log('Listening for zmq requesters...');
});

// Close the responder when the Node process ends
process.on('SIGINT', () => {
    console.log('Shutting down...');
    responder.close();
});